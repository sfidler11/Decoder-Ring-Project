...
const substitutionModule = (function () {
  ...
  function substitution(input, alphabet, encode = true) {
    ...
    //checking for initial conditions, returns false if they are not met
    if (!alphabet || !(alphabet.length === 26)) return false;
    for (let i = 0; i < alphabet.length; i++){
      if (alphabet.includes(alphabet[i], (i + 1))) return false;
    }

    //this is the array that is accessed later to encode the message
    ...
    //inputs the alphabet to the corresponding letter in the codedArray. ex {letter: "a"} => { letter: 'a', code: 'x' }
    for (let i = 0; i < alphabet.length; i++){
    ...
    //to encode
      ...
      for (let i = 0; i < lowerCase.length; i++){
        ...
        let found = codedArray.find((object) => object.letter === lowerCase[i]);
        result += found.code;
        }
      }
    }
